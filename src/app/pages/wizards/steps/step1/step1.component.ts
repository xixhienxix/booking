...
export class Step1Component implements OnInit, OnDestroy {
  ...
  promoValidationStatus: 'idle' | 'valid' | 'invalid' = 'idle';
  ...
  @Output() promoValidated = new EventEmitter<Promos | null>();
  ...
  ngOnInit() {
    ...
    this.formGroup.statusChanges.subscribe(status => {
      ...
      this.updateParentModel({}, this.formGroup.valid); // always notify validity changes
    });
  ...
  emitFormValues() {
    ...
    const promoOk = !codigoPromo?.trim() || this.promoValidationStatus === 'valid';
    const formValid = this.formGroup.valid && promoOk;
    ...
    this.updateParentModel({
      ...
      ninos,
      hotel
    }, formValid);  // ← passes false to parent if promo code is typed but invalid
  }
  ...
  checkPromoCode(): boolean {
    const code = this.formGroup.get('codigoPromo')?.value?.trim().toUpperCase();

    if (!code) {
      this.validatedPromo = null;
      this.promoValidationStatus = 'idle';
      this.promoValidated.emit(null);
      this._disponibilidadService.changeValidatedPromo(null); // ← direct write
      return true;
    }

    if (!this.intialDate || !this.endDate) {
      this.promoValidationStatus = 'invalid';
      this.promoValidationMessage = 'Selecciona las fechas antes de aplicar el código.';
      return false;
    }

    const result = this._promoValidationService.validatePromo(
      code,
    ...
    if (result.valid && result.promo) {
      ...
      this.promoValidationStatus = 'valid';
      this.promoValidationMessage = `Promoción "${result.promo.nombre}" aplicada.`;
      this.promoValidated.emit(result.promo);
      this._disponibilidadService.changeValidatedPromo(result.promo); // ← direct write
      return true;
    } else {
      this.validatedPromo = null;
      this.promoValidationStatus = 'invalid';
      this.promoValidationMessage = result.reason ?? 'Código no válido.';
      this.promoValidated.emit(null);
      this._disponibilidadService.changeValidatedPromo(null); // ← direct write
      return false;
    }
  ...
  clearPromo(): void {
    this.formGroup.get('codigoPromo')?.setValue('');
    this.validatedPromo = null;
    this.promoValidationStatus = 'idle';
    this.promoValidationMessage = '';
    this.promoValidated.emit(null);
    this._disponibilidadService.changeValidatedPromo(null); // ← add this
    this.emitFormValues();
  }
